...
#[test]
fn test_uvm_mock() {
    let src = fs::read_to_string("tests/uvm/uvm_simple_test.sv").expect("Could not read uvm_simple_test.sv");
...
fn test_uvm_complete() {
    ...
    let test_src = fs::read_to_string("tests/uvm/uvm_complete_test.sv").expect("Could not read uvm_complete_test.sv");
...
fn test_uvm_hello_world() {
    let uvm_pkg = fs::read_to_string("uvm-1.2/src/uvm_pkg.sv").expect("Could not read uvm_pkg.sv");
    let test_src = fs::read_to_string("uvm-1.2/examples/simple/hello_world/hello_world.sv").expect("Could not read hello_world.sv");
